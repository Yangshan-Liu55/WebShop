...
<html lang="en">    
  ...
  <body>
    ...
    <div align="center">
      <h3>Order List</h3>      
    <!-- information for the order email -->
    <?php 
    if (isset($_SESSION["user"]))
    { $mailTo=$_SESSION["userEmail"];
    
      if(isset($_SESSION["shopping_cart"])) 
      { $order="Here is your order list: \n";
        $summary=0;
        foreach($_SESSION["shopping_cart"] as $keys => $values)
        { // message information for the order email
          $order.="\nItem: ".$values['item_name']. "\n";
          $order.="Quantity: ".$values['item_quantity']. "\n";
          $order.="Total: ".$values['item_unit'].number_format($values['item_quantity'] * $values['item_price'], 2). "\n";
          $summary = $summary+$values['item_quantity'] * $values['item_price'];    
        }
        $order.="\n    Summary: " . $_SESSION["shopping_cart"]['item_unit'] .number_format($summary, 2). "\n";      
      }
      else{ $order="You haven't ordered anything from us.". "\n"."Welcome to Coco-Clothes again!"; }
    }
    else { echo '<script>alert("Please log in first!")</script>';}       
    ?>

      <form method="post" action="sendOrder.php" align=right>       
        <input type='hidden' name='hidden_to' value="<?php echo $mailTo; ?>">
        <input type='hidden' name='hidden_message' value="<?php echo $order; ?>">        
        <input type='submit' name='sendOrder' class='btn btn-outline-dark' value='Send Order'>
      </form>  
      ...
      <div class="m-1"> 
        <table class="table table-bordered">  
          ...
          <tr><td>
            ...
            <td><a class="btn btn-primary" href="shoppingCart.php?action=delete&id=<?php echo $values["item_id"]; ?>">Remove</a></td></tr> 
          <?php  
